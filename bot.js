...
function calculate_gas_price(action, amount){
  if (action==="buy"){
    return web3.utils.toWei(amount.add(1000000000).toString(), 'wei')
  }else{
    return web3.utils.toWei(amount.sub(1000000000).toString(), 'wei')
  }
}
...
const buyToken = async(account,tokenContract,gasLimit,gasPrice)=>{
  ...
  console.log(`value: ${amountIn}, gasLimit: ${gasLimit}, gasPrice: ${gasPrice}`)
    ...
    if(value>10) {
      ...
      if(result.length>0){
        ...
        if(result[1].length>0){
          ...
          console.log(`Acc: ${account.address}, Gas Limit: ${transaction.gasLimit}, Buy Gas Price: ${buyGasPrice}`);
          ...
          console.log(`Acc: ${account.address}, Gas Limit: ${transaction.gasLimit}, Sell Gas Price: ${sellGasPrice}`);
